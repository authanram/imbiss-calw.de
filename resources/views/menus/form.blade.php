<div class="form-group {{ $errors->has('menu_category_id') ? 'has-error' : ''}}">
    ...
    <div class="col-md-6">
        <input class="form-control" type="number" name="number" type="number" id="number" value="{{ $menu->number ?? ''}}" required>
        {!! $errors->first('number', '<p class="help-block">:message</p>') !!}
    </div>
</div><div class="form-group {{ $errors->has('name') ? 'has-error' : ''}}">
    <label for="name" class="col-md-4 control-label">{{ 'Name' }}</label>
    <div class="col-md-6">
        <input class="form-control" name="name" type="text" id="name" value="{{ $menu->name ?? ''}}" required>
        {!! $errors->first('name', '<p class="help-block">:message</p>') !!}
    </div>
</div><div class="form-group {{ $errors->has('note') ? 'has-error' : ''}}">
    <label for="note" class="col-md-4 control-label">{{ 'Note' }}</label>
    <div class="col-md-6">
        <input class="form-control" name="note" type="text" id="note" value="{{ $menu->note ?? ''}}" >
        {!! $errors->first('note', '<p class="help-block">:message</p>') !!}
    </div>
</div><div class="form-group {{ $errors->has('price') ? 'has-error' : ''}}">
    <label for="price" class="col-md-4 control-label">{{ 'Price' }}</label>
    <div class="col-md-6">
        <input class="form-control" type="number" step="any" name="price" type="text" id="price" value="{{ $menu->price ?? ''}}" required>
...
<div class="form-group">
    <div class="col-md-offset-4 col-md-4">
        <input class="btn btn-primary btn-sm" type="submit" value="{{ $submitButtonText ?? 'Create' }}">
    </div>
</div>
