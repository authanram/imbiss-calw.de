<div class="form-group {{ $errors->has('name') ? 'has-error' : ''}}">
    ...
    <div class="col-md-6">
        <input class="form-control" name="name" type="text" id="name" value="{{ $menucategory->name ?? ''}}" required>
        {!! $errors->first('name', '<p class="help-block">:message</p>') !!}
    </div>
</div><div class="form-group {{ $errors->has('order') ? 'has-error' : ''}}">
    <label for="order" class="col-md-4 control-label">{{ 'Order' }}</label>
    <div class="col-md-6">
        <input class="form-control" type="number" name="order" type="number" id="order" value="{{ $menucategory->order ?? ''}}" required>
